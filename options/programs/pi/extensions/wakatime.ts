...
function getWakaHome(): string {
  return (
    process.env.WAKATIME_HOME || join(process.env.HOME || ".", ".wakatime")
  );
}
...
function readProjectState(projectFolder: string): ProjectState {
  try {
    return JSON.parse(
      readFileSync(getStateFile(projectFolder), "utf8"),
    ) as ProjectState;
  } catch {
    return {};
  }
...
function shouldSendHeartbeat(projectFolder: string, force: boolean): boolean {
  ...
  if (
    ...
    now - lastHeartbeatAttemptAt < HEARTBEAT_RETRY_SECONDS
  )
    return false;
...
async function ensureCli(pi: ExtensionAPI): Promise<boolean> {
  ...
  try {
    const version = await pi.exec("wakatime-cli", ["--version"], {
      timeout: 5000,
    });
    const versionOutput = `${version.stdout}\n${version.stderr}`;
    if (
      version.code !== 0 ||
      /failed to parse config|permission denied/i.test(versionOutput)
    ) {
      cliAvailable = false;
      cliStatus =
        sanitizeOutput(versionOutput) ||
        `wakatime-cli --version exited with ${version.code}`;
      return cliAvailable;
    }

    const config = await pi.exec("wakatime-cli", ["--config-read", "api_key"], {
      timeout: 5000,
    });
    const configOutput = `${config.stdout}\n${config.stderr}`;
    if (
      config.code !== 0 ||
      /api key not found|api_key.*empty|failed to parse config|permission denied/i.test(
        configOutput,
      )
    ) {
      cliAvailable = false;
      cliStatus =
        sanitizeOutput(configOutput) ||
        `wakatime-cli config check exited with ${config.code}`;
      return cliAvailable;
    }
...
function extractPatchChanges(
  event: GenericToolResultEvent,
  cwd: string,
): TrackedFileChange[] {
  const changes: TrackedFileChange[] = [];
  const details = asRecord(event.details);
  const patches =
    asArray(details?.patches) ??
    asArray(details?.changes) ??
...
function extractMultieditChanges(
  event: GenericToolResultEvent,
  cwd: string,
): TrackedFileChange[] {
  const changes: TrackedFileChange[] = [];
...
function queueChange(change: TrackedFileChange): void {
  ...
  if (
    !change.entity ||
    (entityType === "file" && !isProbablyFile(change.entity))
  )
    return;
...
async function sendHeartbeat(
  ...
  try {
    ...
    if (
      result.code !== 0 ||
      /"level":"error"|failed to parse config|api key not found|permission denied/i.test(
        output,
      )
    ) {
      lastHeartbeatError =
        sanitizeOutput(output) || `wakatime-cli exited with ${result.code}`;
      return false;
    }
...
export default function wakatimeExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("wakatime-status", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!hasCli) {
        lines.push(
          "Install/configure wakatime-cli and make sure ~/.wakatime.cfg is readable.",
        );
      }
