  ...
  options.galaxy = {
    mail = {
      ...
      accounts = {
        me = lib.mkOption {
          type = lib.types.str;
          default = "galaxy/mail/accounts/me";
          readOnly = true;
        };
        bot = lib.mkOption {
          type = lib.types.str;
          default = "galaxy/mail/accounts/bot";
          readOnly = true;
        };
        komputah = lib.mkOption {
          type = lib.types.str;
          default = "galaxy/mail/accounts/komputah";
          readOnly = true;
        };
      };
    };
  ...
  config = lib.mkMerge [
    {
      age.secrets = {
        ${mail.accounts.me} = {
          rekeyFile = ../../secrets/galaxy/mail/accounts/me.age;
        };
        ${mail.accounts.bot} = {
          rekeyFile = ../../secrets/galaxy/mail/accounts/bot.age;
        };
        ${mail.accounts.komputah} = {
          rekeyFile = ../../secrets/galaxy/mail/accounts/komputah.age;
        };

        ${rspamdPassword} = {
          rekeyFile = ../../secrets/galaxy/mail/rspamd/password.age;
          intermediary = true;
        };
        ${rspamdWorkerController} = {
          rekeyFile = ../../secrets/galaxy/mail/rspamd/workerController.age;
          generator = {
            dependencies = {
    ...
    (lib.mkIf mail.enable {
      galaxy = {
        proxy.rules = [
          ...
          {
            name = "rspamd";
            from.host = "rspamd.${domain}";
            to.http = "http://127.0.0.1:11334";
          }
        ];

        backup.paths = [ "/var/lib/maddy" ];
        acme.extraCertificates.${hostname}.reloadServices = [ "maddy.service" ];
      };
      ...
      services = {
        maddy = {
          ...
          ensureCredentials = {
            "me@${domain}".passwordFile = age.secrets.${mail.accounts.me}.path;
            "bot@${domain}".passwordFile = age.secrets.${mail.accounts.bot}.path;
            "komputah@${domain}".passwordFile = age.secrets.${mail.accounts.komputah}.path;
          };
