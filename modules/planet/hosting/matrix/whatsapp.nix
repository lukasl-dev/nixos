...
let
  ...
  pickleKey = atlas.secrets.universe [
    "matrix"
    "whatsapp"
    "pickleKey"
  ];
...
in
{
  config = lib.mkIf matrix.enable {
    age.secrets = {
      ${pickleKey} = {
        rekeyFile = ../../../.. + "/secrets/${pickleKey}.age";
      ...
      ${environment} = {
        ...
        generator = {
          ...
          script =
            ...
            ''
              pickle_key="$(${decrypt} \
                ${lib.escapeShellArg deps.pickleKey.file})"
              printf \
                'MAUTRIX_WHATSAPP_ENCRYPTION_PICKLE_KEY=%s\n' \
                "$pickle_key"
            '';
        };
      };
    ...
    services = {
      ...
      mautrix-whatsapp = {
        ...
        settings = {
          ...
          homeserver = {
            address = "http://${listenAddress}:${toString matrixPort}";
            inherit domain;
          };
          ...
          bridge = {
            ...
            permissions = {
              ${domain} = "user";
              "@${steward.user.name}:${domain}" = "admin";
            };
          };
    ...
    systemd.services = {
      mautrix-whatsapp-registration = {
        ...
        script = ''
          ...
          ln -sfn \
            '${registrationFile}' \
            '${appserviceDir}/whatsapp-registration.yaml'
        '';
      };
