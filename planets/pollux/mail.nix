...
let
  ...
  me = "galaxy/lukasl-dev/mail/acounts/me";
  bot = "galaxy/lukasl-dev/mail/accounts/bot";
  komputah = "galaxy/lukasl-dev/mail/accounts/komputah";
...
in
{
    ...
    lukasl-dev.proxy.rules = [
      {
        ...
        from = config.services.go-autoconfig.settings.domain;
        http.to = "http://localhost${config.services.go-autoconfig.settings.service_addr}";
      }
      {
        type = "https";
        name = "rspamd";
        from = "rspamd.${lukasl-dev.domain}";
        http.to = "http://127.0.0.1:11334";
      }
    ];
  ...
  services = {
    maddy = {
      ...
      ensureCredentials = {
        "me@${lukasl-dev.domain}".passwordFile = age.secrets.${me}.path;
        "bot@${lukasl-dev.domain}".passwordFile = age.secrets.${bot}.path;
        "komputah@${lukasl-dev.domain}".passwordFile = age.secrets.${komputah}.path;
      };
